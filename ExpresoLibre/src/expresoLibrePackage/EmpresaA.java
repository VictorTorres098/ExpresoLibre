...
public class EmpresaA {
	...
	public void iniciarViaje(String matricula) {
		if(flotaTrasportes.get(matricula).consultarDisponibilidad() && flotaTrasportes.get(matricula).remolqueCargado()) {
			flotaTrasportes.get(matricula).iniciarViaje(); //si es false = true. si es true = false
		}else {
	...
	public void finalizarViaje(String matricula) {
		if(flotaTrasportes.get(matricula).consultarSiEstaDeViaje()) { //si no esta disponible quiere decir que esta en viaje, pero se niega para que entre correctamente ak if
			flotaTrasportes.get(matricula).vaciarCarga();				//vacia la carga
			flotaTrasportes.get(matricula).finalizarViaje(); //lo podria haber echo en una funcion?? preguntar
		}else {
	...
	public double obtenerCostoViaje(String matricula) {
		if(flotaTrasportes.get(matricula).consultarDisponibilidad()) {
			return flotaTrasportes.get(matricula).devolverCosto();
		}else {
	...
	//solo para test devuelva la cantidad de volumen de la carga
	public double obtenerVolumenDeCarga(String matricula) {
		return flotaTrasportes.get(matricula).obtenerVolumen();
	}
	//solo para test saber el costo del viaje
	public double costoTrasporte(String matricula) {
		String test = "hola";
		return flotaTrasportes.get(matricula).devolverCosto();
